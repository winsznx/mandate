...
import { GRID_A_POLICY, describePolicy } from "../src/policy.js";
...
describe("Tight Grid", () => {
  ...
  it("publishes its policy even though the strategy is pending", () => {
    ...
    const published = describePolicy(GRID_A_POLICY) as Record<string, unknown>;

    // #then the parameters a trial would bind to already exist
    expect(published["policyId"]).toBe(GRID_A_POLICY.policyId);
  ...
  it("refuses to deliberate, naming itself and the skill", async () => {
    ...
    await expect(executor.propose(REQUEST)).rejects.toThrow(/grid-a/);
  });
});
