...
import { GRID_B_POLICY, describePolicy } from "../src/policy.js";
...
describe("Adaptive Grid", () => {
  it("registers under the GRID category", () => {
  ...
  it("advertises the skill a trial harness will call", () => {
    ...
    expect(executor.skills.map((skill) => skill.id)).toEqual(["adjust-grid"]);
  });

  it("publishes its policy even though the strategy is pending", () => {
    // #given the published policy document
    const published = describePolicy(GRID_B_POLICY) as Record<string, unknown>;

    // #then the parameters a trial would bind to already exist
    expect(published["policyId"]).toBe(GRID_B_POLICY.policyId);
  ...
  it("refuses to deliberate, naming itself and the skill", async () => {
    ...
    await expect(executor.propose(REQUEST)).rejects.toThrow(/grid-b/);
  });
});
