...
import { REBALANCING_B_POLICY, describePolicy } from "../src/policy.js";
...
describe("Wide Range Manager", () => {
  ...
  it("publishes its policy even though the strategy is pending", () => {
    ...
    const published = describePolicy(REBALANCING_B_POLICY) as Record<string, unknown>;

    // #then the parameters a trial would bind to already exist
    expect(published["policyId"]).toBe(REBALANCING_B_POLICY.policyId);
  ...
  it("refuses to deliberate, naming itself and the skill", async () => {
    ...
    await expect(executor.propose(REQUEST)).rejects.toThrow(/rebalancing-b/);
  });
});
