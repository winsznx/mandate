...
export const CATEGORIES: readonly Category[] = [
  {
    ...
    key: "REBALANCING",
    name: "Rebalancing",
    task: "Keep my capital allocated the way I chose",
    decision:
      "When a lending allocation drifts away from its target weights, decide whether topping up the underweight market is worth the cost of the move.",
    evidenceVector: [
      "protocol and markets in the allocation",
      "target weights",
      "drift trigger",
      "rebalance count",
      ...
      "allocation value against a declared baseline",
      "holds where only a withdrawal would have closed the gap",
      "policy violations",
      "evidence window",
      "capital range",
      "trial history",
    ...
    authorityShape:
      "One Venus vToken per market in the allocation, the single selector mint(uint256), and a spend cap on the underlying. That selector takes one integer and credits msg.sender, so no recipient is reachable from calldata.",
    caution:
      "This rebalances by top-up only. Reducing an overweight market needs redeemUnderlying, which is withheld: it is bounded in reach, but withdrawing collateral can drive a borrowing account below a health factor of one, and no target-selector-spend triple can express a health-factor floor.",
  },
  {
    slug: "grid-trading",
    key: "GRID",
