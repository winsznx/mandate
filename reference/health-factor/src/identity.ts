 ...
 *
 * A trial receipt names a `referenceModelHash` so that a reader can ask which
 * model said this, and so that a change to the model supersedes the receipts it
 * backed rather than silently inheriting them. Hashing the source is the only
 * form of that claim which cannot drift from what actually ran; a hand-bumped
 * version string is a promise, and this is a measurement.
 *
...
const SOURCE_ROOT = "reference/health-factor/src";
 ...
 */
const SOURCE_FILES = ["scale.ts", "accounting.ts", "model.ts", "index.ts"] as const;
 ...
 */
export function referenceInputsHash(observation: RawVenusObservation): Hex {
